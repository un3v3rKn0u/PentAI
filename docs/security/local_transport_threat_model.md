...
| Caller substitutes local-model artifact identity or filesystem trust rules | Verification of unapproved runtime/model bytes or a widened installation boundary | A closed built-in manifest resolves without caller selectors and pins ADR 0008's exact macOS arm64 release, runtime closure, model revision, sizes, digests, layout, APFS, ownership, permissions, links, hashing, and inactive lifecycle; altered or authority-bearing documents deny | Authenticated registry activation, installed-file verification, receipts, grants, and execution remain future work |
