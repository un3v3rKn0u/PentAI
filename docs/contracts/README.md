...
- `ai-provider-configuration-snapshot-command-v1.schema.json` — inert authenticated
  production command consumed only by the trusted inactive-snapshot producer and bound
  to one exact current registry activation and configuration.
- `ai-provider-configuration-snapshot-receipt-v2.schema.json` — source-bound immutable
  production receipt that cannot activate a meter or replace snapshot receipt v1.
...
- `ai-runtime-meter-identity-v1.schema.json` — inert provider-configuration and
...
- `ai-runtime-meter-implementation-v1.schema.json` — inert implementation/version,
...
- `orchestration-task-failure-command-v3.schema.json` — exact attempt-three failure
  command bound to lease-consumption v3 and an exact checkpoint-v3 head or absence.
- `orchestration-task-failure-receipt-v3.schema.json` — immutable terminal-attempt
  classification receipt with no retry or execution authority.
...
- `orchestration-terminal-disposition-command-v1.schema.json` — exact failed-attempt-v3
...
- `orchestration-task-completion-command-v3.schema.json` — version-exact attempt-three
  successful-completion request bound to the current lease/checkpoint lineage.
...
| Engagement Manifest v3 | Product Safety Lead | Contract Maintainer | Product Owner, Policy Maintainer, Security Reviewer |
| Policy IR v1 | Policy Maintainer | Contract Maintainer | Core Maintainer, independent Security Reviewer |
| Policy IR v2 | Policy Maintainer | Contract Maintainer | Core Maintainer, independent Security Reviewer |
